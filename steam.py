...
class steam():
    ...
    def calc(self):
        ...
        superheated_data = np.loadtxt('superheated_water_table.txt', skiprows=1, unpack=True)
        tcol, hcol, scol, pcol = superheated_data[0], superheated_data[1], superheated_data[2], superheated_data[3]
        ...
        if self.T is not None:
            ...
            if self.x<=1.0: #manual interpolation
                ...
                self.region='Superheated'
                self.T = float(griddata((hcol, pcol), tcol, (self.h, Pbar))) #use griddata to interpolate with h & P the superheated table
                self.s = float(griddata((tcol, pcol), scol, (self.T, Pbar))) #use griddata to interpolate with h & P the superheated table
        elif self.s!=None:
            ...
            if self.x<=1.0: #manual interpolation
                ...
                self.region = 'Superheated'
                self.T = float(griddata((hcol, pcol), tcol, (self.h, Pbar))) #use griddata to interpolate with s & P the superheated table
                self.h = hf + self.x * (hg - hf) #use griddata to interpolate with s & P the superheated table
        #endregion
