...
# we used Dr. smays stem files to help write this program
# brandon and I shared screens in order to complete this code

#region class definitions
class ResistorNetwork_2():
    #region constructor
    def __init__(self):
        """
    ...
    def GetVSourceByName(self, name):
        for v in self.VSources:
            if v.Name == name:
                return v
        return None  # Or raise an exception if the source is not found
    ...
    def BuildNetworkFromFile(self, filename):
        ...
        #Must change path to ResistorNetwork_2.txt
        FileTxt = open(r"C:\Users\textt\iCloudDrive\Spring 2024\Computer_Methods\Github\HW6\ResistorNetwork_2.txt","r").read().split('\n')  # reads from file and then splits the string at the new line characters
    ...
    def GetKirchoffVals(self, i):
        ...
        # KVL for Loop 1 (acdb)
        equations.append(-16 + i[0] * 2 + i[2] * 1 + i[1] * 2)

        # KVL for Loop 2 (decf)
        equations.append(-32 + i[4] * 4 + i[3] * 2)

        # KCL at Node c
        equations.append(i[0] - i[2] - i[4])

        # KCL at Node d
        equations.append(i[2] - i[1] - i[3])

        # Additional equation needed for the current through the 5Ω resistor in parallel with 32V source (ed)
        equations.append(i[4] - (32 / 5))
...
def main():
    ...
    :return: nothing
    """
    Net = ResistorNetwork_2()  #Instantiate a resistor network object
    # Must change path to ResistorNetwork_2.txt
    Net.BuildNetworkFromFile(r"C:\Users\textt\iCloudDrive\Spring 2024\Computer_Methods\Github\HW6\ResistorNetwork_2.txt") #call the function from Net that builds the resistor network from a text file
