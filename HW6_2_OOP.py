...
class Pipe():
    ...
    def frictionHeadLoss(self):  # calculate headloss through a section of pipe in m of fluid
        ...
        hl = (ff * self.length / self.d) * (self.fluid.rho * self.vel**2) / (2 * g) # calculate the head loss in m of water
        return hl
...
class PipeNetwork():
    ...
    def findFlowRates(self):
        ...
        def fn(q):
            ...
            L= np.array(self.getNodeFlowRates())  # call the getNodeFlowRates function of this class
            ...
            L = np.concatenate((L, np.array(self.getLoopHeadLosses())))  # call the getLoopHeadLosses function of this class
            return L
